...
```jsonc
{
  ...
  "requires": { "chatDisplay": 1 },   // ไม่บังคับ — 0=Flat, 1=Bubbles, 2=Document
                                       // ใส่เฉพาะธีมที่วาดโครงหน้าเองแบบผูกกับ
                                       // โหมดใดโหมดหนึ่งจริงๆ (เช่น line-chat
                                       // วาดบับเบิ้ลเอง ถ้าอยู่ผิดโหมดหน้าตาพัง)
                                       // ธีมที่ใช้ได้ทุกโหมดอยู่แล้ว (เช่น
                                       // thai-novel-reader) ไม่ต้องใส่ฟิลด์นี้เลย

  "checkboxTextVar": "text",          // ไม่บังคับ, ค่าเริ่มต้น "text" — ชื่อ
                                       // key ใน vars ที่จะใช้คำนวณ contrast ของ
                                       // เครื่องหมายถูก checkbox (ดูหัวข้อ 5)
                                       // เปลี่ยนได้ถ้าธีมตั้งชื่อสีข้อความหลัก
                                       // ว่าอย่างอื่น (เช่น "ui-text")

  "fontImports": [                    // ไม่บังคับ — URL ของ Google Fonts (หรือ
    "https://fonts.googleapis.com/css2?family=...&display=swap"  // สไตล์ชีตอื่นที่ใช้ @import ได้) เอาไว้ก่อน @font-face
  ],                                  // เสมอ — ห้ามใส่ @import ใน theme.css เอง
                                       // (ต้องอยู่บรรทัดแรกสุดของสไตล์ชีต ซึ่ง
                                       // เครื่องยนต์คุมลำดับให้จากช่องนี้แทน)

  "fonts": [                          // @font-face จากไฟล์ .ttf ของตัวเอง (ถ้ามี)
    {
      "family": "ชื่อฟอนต์",
      "src": [
  ...
  "modes": {                          // อย่างน้อย 1 โหมด — key คือ id ภายใน (lowercase)
    "mode-id": {
      ...
      "vars": { /* ตารางกลุ่ม A ทั้ง 12 ตัว — ใส่ให้ครบทุกตัว (หรือชุดตัวแปร
                   ของธีมเอง ถ้าไม่ได้ยึดรูปแบบ 12 ตัวนี้ — ดูหัวข้อ 5) */ },
      "extraCss": "/* ไม่บังคับ — CSS เพิ่มที่ใช้เฉพาะโหมดนี้โหมดเดียว
                      เช่น เอฟเฟกต์พิเศษที่โหมดอื่นไม่มี ต่อท้ายก้อน vars
                      อัตโนมัติ ไม่ต้องกังวลเรื่องลำดับ <style> เอง */",
...
| **เครื่องหมายถูก checkbox ซ้อนกัน 2 อัน** | ติ๊ก checkbox แล้วเห็นเครื่องหมายถูกเบลอๆ ซ้อนทับกัน หรือสีผิดไปจากธีมเลย | core วาดเครื่องหมายถูกของตัวเองอยู่แล้วผ่าน `::before` (clip-path shape ใน style.css) โดยสีคำนวณจาก `--SmartThemeCheckboxBgColorR/G/B` ที่ core set เป็น **inline style บน `:root`** ทุกครั้งที่มีการสลับธีม native (ไม่ใช่แค่ตอนโหลดครั้งแรก) — ถ้าธีมไปวาดเครื่องหมายถูกเองซ้ำผ่าน `::after` จะกลายเป็น 2 อันซ้อนกัน และถึงจะไม่วาดเองแล้ว ถ้าลืม sync `--SmartThemeCheckboxBgColorR/G/B` (คำนวณจาก hex ของ `checkboxTextVar`) สีก็จะยังผิดอยู่ดีเพราะ core เขียนทับใหม่ทุกครั้ง | **อย่าวาด `::after`/`::before` เองเด็ดขาด** ปล่อยให้ core วาด แค่ sync `--SmartThemeCheckboxBgColorR/G/B` จาก `checkboxTextVar` ไว้ **ที่ `:root` เท่านั้น** (ไม่ใช่ที่ `input[type="checkbox"]` — ประกาศผิด scope แล้วจะไม่มีผลอะไรเลยเพราะสูตรคำนวณของ core อ่านค่าที่ `:root` scope) เครื่องยนต์ทำให้อัตโนมัติแล้วผ่าน `checkboxTextVar` (ดูหัวข้อ 4) — เผื่อไว้ด้วยว่าอาจมีธีม native อื่นที่ผู้ใช้เลือกคู่กันซึ่งมี `::after` ของตัวเอง จึงควรมี `input[type="checkbox"]::after { content: none !important; }` กันไว้เสมอ |
| **เนื้อหาเยื้องซ้ายจากจุดกึ่งกลางจอ 2-8px** | การ์ด/บับเบิ้ลที่ตั้งใจ `margin: 0 auto` ดูเหมือนจะกึ่งกลาง แต่จริงๆ เยื้องซ้ายนิดหน่อยเทียบกับขอบจอ | `#chat` คือ element ที่ scroll ได้จริง scrollbar ของมันกินความกว้างจากขอบขวาข้างเดียว ทำให้ "พื้นที่เนื้อหา" ของ `#chat` แคบกว่ากล่องจริงเท่ากับความกว้าง scrollbar แบบไม่สมมาตร — `margin:auto` คำนวณจุดกึ่งกลางจากพื้นที่ที่แคบกว่านั้น ไม่ใช่จากจอจริง | ใส่ `scrollbar-gutter: stable both-edges;` ให้ `#chat` — จองพื้นที่ว่างฝั่งซ้ายเท่ากับ scrollbar ฝั่งขวา ทำให้กล่องกลับมาสมมาตรแล้ว `margin:auto` กึ่งกลางถูกจริง (เบราว์เซอร์เก่าที่ไม่รองรับแค่กลับไปเป็นพฤติกรรมเดิม ไม่ได้แย่ลง) |
